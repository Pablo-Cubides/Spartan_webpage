// Coach Triarvon - Coaches List API
...
import { COACHES, type CoachType } from '@/lib/coach-triarvon/config/coaches.config';
...
const handler = async (request: NextRequest) => {
    ...
    const rawEnabled = user.triarvonProfile.enabledCoaches as CoachType[];
    ...
    const coaches = enabledCoachIds.map(coachId => {
        ...
        const conversation = user.triarvonProfile!.conversations.find(c => c.coachType === coachId);
    ...
    return NextResponse.json({
        hasProfile: true,
        onboardingDone: user.triarvonProfile.onboardingDone,
        coaches,
