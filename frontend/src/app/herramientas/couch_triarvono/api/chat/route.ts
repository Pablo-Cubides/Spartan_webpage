...
import { getChatResponse, runStrategistAnalysis, type TriarvonProfileContext, type ChatMessage } from '@/lib/coach-triarvon/gemini';
import { canSendMessage, recordMessageSent } from '@/lib/coach-triarvon/credits';
import { encryptMessage, decryptMessage } from '@/lib/coach-triarvon/encryption';
import { checkMessageSafety } from '@/lib/coach-triarvon/safety';
import { COACH_SETTINGS, type CoachType, COACHES } from '@/lib/coach-triarvon/config/coaches.config';
...
const handler = async (request: NextRequest) => {
    ...
    if (!user.triarvonProfile?.onboardingDone) {
    ...
    if (!user.triarvonProfile.enabledCoaches.includes(coachType)) {
        return NextResponse.json(
    ...
    const profileContext: TriarvonProfileContext = {
        mainGoal: user.triarvonProfile.mainGoal,
        subGoals: user.triarvonProfile.subGoals,
        levels: user.triarvonProfile.levels as Record<string, string> | undefined,
        restrictions: user.triarvonProfile.restrictions as Record<string, string> | undefined,
        preferences: user.triarvonProfile.preferences as Record<string, boolean> | undefined,
        currentFocuses: user.triarvonProfile.currentFocuses
    };

    // Get strategist guidance if available
    const strategistGuidance = user.triarvonProfile.strategistPlan as Record<string, unknown> | undefined;
    ...
    if (newMessageCount >= COACH_SETTINGS.STRATEGIST_THRESHOLD &&
        ...
        triggerStrategist(user.triarvonProfile.id, coachType as CoachType, profileContext).catch(console.error);
    }
...
async function triggerStrategist(
    profileId: number,
    coachType: CoachType,
    profileContext: TriarvonProfileContext
) {
    try {
        ...
        await prisma.triarvonProfile.update({
            where: { id: profileId },
            data: {
        ...
        console.error('[Coach Triarvon] Strategist error:', error);
    }
}
