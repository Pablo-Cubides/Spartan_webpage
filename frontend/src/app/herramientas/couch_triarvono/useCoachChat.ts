...
// Coach Triarvon - Custom Hook for Chat State Management
...
export function useCoachChat(initialCoaches: Coach[]): UseCoachChatReturn {
    ...
    const fetchHistory = useCallback(async (coachId: string) => {
        ...
        try {
            ...
            const res = await fetch(`/herramientas/couch_triarvono/api/chat/history?coachType=${coachId}`, {
    ...
    const closeWelcomeModal = useCallback(async () => {
        ...
        try {
            const token = getTokenCookie();
            await fetch('/herramientas/couch_triarvono/api/welcome', {
                method: 'POST',
                headers: {
    ...
    const sendMessage = useCallback(async (content: string) => {
        ...
        try {
            const token = getTokenCookie();
            const res = await fetch('/herramientas/couch_triarvono/api/chat', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
