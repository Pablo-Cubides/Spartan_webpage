...
import { analyzeClothingWithGemini } from '@/lib/asesor-estilo/ai/gemini';
...
export async function POST(req: Request): Promise<Response> {
    ...
    try {
        ...
        // 3. Check cache (use clothing-specific cache key)
        const imageBuffer = await getImageBuffer(imageUrl);
        const imageHash = calculateImageHash(imageBuffer);
        const cacheKey = generateAnalysisCacheKey(`clothing_${imageHash}`, effectiveLocale);

        const cached = await getCached(cacheKey);
        if (cached) {
            await appendLog({
                phase: 'api.analyze-clothing.cache_hit',
            ...
            return NextResponse.json(
                {
                    ...
                    message: 'Debes verificar tu email para usar esta herramienta.',
                },
                { status: 403 }
            );
        }
        ...
        // 5. Perform CLOTHING analysis (different from face analysis)
        const analysis = await analyzeClothingWithGemini(imageUrl, effectiveLocale);
        ...
        // 8. Log success
        await appendLog({
            phase: 'api.analyze-clothing.result',
            imageUrl: APP_CONFIG.compliance.PRIVACY_MODE ? '[redacted]' : imageUrl,
            success: true,
            bodyOk: analysis.bodyOk,
        ...
        // Log error
        await appendLog({
            phase: 'api.analyze-clothing.error',
