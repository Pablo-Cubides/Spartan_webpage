// Coach Espartano - Message Encryption
...
function getEncryptionKey(): Buffer {
    ...
    if (!key) {
        ...
        if (process.env.NODE_ENV === 'development') {
            console.warn('[Coach Espartano] Using default encryption key - NOT SAFE FOR PRODUCTION');
            return crypto.scryptSync('dev-key-spartan-coach', 'salt', 32);
        }
        throw new Error('COACH_ENCRYPTION_KEY environment variable is required');
    }

    // Derive a 32-byte key from the provided key
    return crypto.scryptSync(key, 'spartan-salt', 32);
}
